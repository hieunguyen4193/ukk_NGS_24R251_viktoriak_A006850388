...
meta.data.batch1 <- subset(meta.data, meta.data$batch == "batch1")

#####----------------------------------------------------------------------#####
##### DEFINE FUNCTION TO RUN DESEQ2
#####----------------------------------------------------------------------#####
run_pipeline_DESEQ2 <- function(condition1, 
                                condition2, 
                                condition.col, 
                                outputdir, 
                                input.metadata){

  input.metadata <- input.metadata[, c("Sample.ID", "Name", "Genotype", "Treatment", condition.col)]
  colnames(input.metadata) <- c("sample", "Name", "Genotype", "Treatment", "condition")
  
  path.to.save.output <- file.path(outputdir, condition.col, sprintf("case_%s_vs_%s", condition1, condition2))
  dir.create(path.to.save.output, showWarnings = FALSE, recursive = TRUE)
  
  filtered.metadata <- subset(input.metadata, input.metadata$condition %in% c(condition1, condition2))
...
for (condition.col in all.conditions){
  run_pipeline_DESEQ2(condition1 = "group1", 
                      ...
                      input.metadata = meta.data.batch1)
}
